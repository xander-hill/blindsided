...
class StorageServiceTests(BackendTestCase):
    def test_initial_commit_assigns_version_and_reserve_status(self):
        ...
        self.assertEqual(judge.vault["auction-1"].version, 1)
        self.assertTrue(judge.vault["auction-1"].reserve_met)

    def test_commit_rejects_stale_versions_and_preserves_existing_state(self):
        judge = make_judge(role="backup")
        judge.vault["auction-1"] = pb2.Auction(
            auction_id="auction-1",
            title="Chronograph",
            version=3,
            bids={"buyer-a": 300.0},
    ...
    def test_commit_merges_bids_and_overwrites_same_buyer(self):
        ...
        self.assertEqual(judge.vault["auction-1"].version, 3)
        self.assertEqual(judge.vault["auction-1"].bids["buyer-a"], 700.0)
        self.assertEqual(judge.vault["auction-1"].bids["buyer-b"], 450.0)
        self.assertTrue(judge.vault["auction-1"].reserve_met)
    ...
    def test_query_filters_by_id_title_and_description(self):
        judge = make_judge(role="backup")
        judge.vault["a-1"] = pb2.Auction(
            auction_id="a-1",
            title="Vintage Camera",
            description="Brass body",
        )
        judge.vault["a-2"] = pb2.Auction(
            auction_id="a-2",
            title="Modern Watch",
