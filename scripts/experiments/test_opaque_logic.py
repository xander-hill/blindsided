...
def run_test():
    ...
    def get_opaque_stats():
        # Using GetStatus (which we updated to return masked data)
        resp = stub.GetStatus(pb2.StatusRequest(auction_id=auc_id))
        # Note: Depending on your exact GetStatus implementation, 
        # you might need to check the 'reserve_met' boolean in the returned auction.
        return resp.auction

    # --- SCENARIO 2: Single User Bidding & Opaque Range ---
    print("\n2️⃣  Xander bids $100...")
    stub.PlaceSecretBid(pb2.BidRequest(auction_id=auc_id, buyer_id="Xander", amount=100.0, expected_version=1))
    ...
    current_v = stub.GetStatus(pb2.StatusRequest(auction_id=auc_id)).auction.version
    stub.PlaceSecretBid(pb2.BidRequest(auction_id=auc_id, buyer_id="Xander", amount=600.0, expected_version=current_v))
    
    # --- SCENARIO 4: Multiple Users & Silent Minimum (Pillar #1 & #2) ---
    print("4️⃣  Beatz bids $200...")
    current_v = stub.GetStatus(pb2.StatusRequest(auction_id=auc_id)).auction.version
    stub.PlaceSecretBid(pb2.BidRequest(auction_id=auc_id, buyer_id="Beatz", amount=200.0, expected_version=current_v))
    ...
    for update in stub.JoinLiveAuction(pb2.AuctionRequest(auction_id=auc_id)):
        ...
        print(f"  [Opaque View] Reserve Met: {update.reserve_status}")
        
        if update.bidder_count == 2 and update.reserve_status == True:
        ...
        print("\n🔨 Dropping the Gavel...")
        stub.DropTheGavel(pb2.GavelRequest(auction_id=auc_id, expected_version=-1))
    ...
    for reveal in stub.JoinLiveAuction(pb2.AuctionRequest(auction_id=auc_id)):
        if reveal.is_revealed:
            ...
            if reveal.winner_id == "Xander" and reveal.final_price == 600.0:
